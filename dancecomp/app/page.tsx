...
export type ViewTab = "emcee" | "backstage" | "import" | "events";

export default function Home() {
  const { user, role, loading: authLoading, signOut } = useAuth();
  const { activeEvent, events, createEvent, switchEvent, loading: eventsLoading } = useEvents();
  const eventSlug = activeEvent?.slug ?? "demo-event";

  const routines = useRoutines(eventSlug);
  const chat     = useChat(eventSlug);

  const [view,       setView]       = useState<ViewTab>(role === "emcee" ? "emcee" : "backstage");
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        <main
          className="flex-1 overflow-y-auto p-4 md:p-7 transition-all duration-300"
            ...
            {view === "import"    && (
              <ImportView
                onImport={async (rows) => {
                  ...
                  await routines.bulkInsert(rows as any);
                  setView("backstage");
                }}
              />
            )}
...
function EventsModal({ events, activeEvent, onSwitch, onCreate, onClose }: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-md rounded-[18px] border p-6"
        ...
        <div className="flex flex-col gap-2 mb-4 max-h-56 overflow-y-auto">
          {events.length === 0 && (
            <div className="font-mono text-[12px] text-gray-600 text-center py-4">No events yet</div>
          )}
          {events.map((ev: any) => (
            <button key={ev.id}
            ...
            >
              <div className="flex-1">
                <div className={`font-semibold text-[14px] ${activeEvent?.id === ev.id ? "text-pink-400" : "text-white"}`}>
                  {ev.name}
                </div>
        ...
        {!creating ? (
          <button
            className="w-full h-11 rounded-[8px] border border-dashed text-[13px] font-medium text-gray-500 hover:text-white hover:border-[var(--border2)] transition-all"
          ...
          <form onSubmit={handleCreate} className="flex flex-col gap-2.5 border-t pt-4"
            style={{ borderColor: "var(--border)" }}>
            <div className="font-mono text-[10px] tracking-[2px] uppercase text-gray-600 mb-1">New Event</div>
            {[
              { key: "name",     val: name,     set: setName,     ph: "Event name (e.g. Spring Showcase 2025)" },
              { key: "date",     val: date,     set: setDate,     ph: "Date (e.g. May 3, 2025)",  },
              { key: "location", val: location, set: setLocation, ph: "Venue / location" },
            ].map(({ key, val, set, ph }) => (
              <input key={key} required placeholder={ph} value={val}
                onChange={e => set(e.target.value)}
                className="h-[46px] rounded-[8px] border px-3.5 text-[14px] outline-none placeholder-gray-600"
                style={{ background: "var(--surface)", borderColor: "var(--border)", color: "var(--text)" }}
              />
            ))}
            <div className="flex gap-2 mt-1">
              <button type="submit" disabled={loading}
                className="flex-1 h-11 rounded-[8px] font-semibold text-[13px] disabled:opacity-40 text-black"
                style={{ background: "var(--green)" }}>
                {loading ? "Creating…" : "Create Event"}
              </button>
              <button type="button"
                className="h-11 px-4 rounded-[8px] border text-[13px] text-gray-500"
                style={{ borderColor: "var(--border)" }}
