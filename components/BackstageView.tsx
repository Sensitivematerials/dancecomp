...
export default function BackstageView({
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      list = list.filter(r =>
        r.number.toLowerCase().includes(q) || r.studio.toLowerCase().includes(q) ||
        r.title.toLowerCase().includes(q)  || r.division.toLowerCase().includes(q)
      );
    }
  ...
  async function handleAdd() {
    ...
    await addRoutine(newR);
    setNewR({ number: "", studio: "", title: "", division: "" });
    setShowAdd(false);
  }

  const borderFor = (s: RoutineStatus) => ({
    stage:     "border-yellow-400/28",
    ready:     "border-emerald-400/26",
    checked:   "border-amber-400/26",
    "not-here":"border-[var(--border)]",
    completed: "border-[var(--border)]",
  }[s]);
  ...
  return (
    <div>
      ...
      <div className="flex gap-2.5 mb-3.5">
        <input
          className="flex-1 h-[52px] rounded-[8px] border px-4 text-[15px] outline-none bg-[var(--card)] placeholder-gray-600 focus:border-[var(--border2)]"
          style={{ borderColor: "var(--border)" }}
          placeholder="Search by number, studio, title…"
          value={search}
          onChange={e => setSearch(e.target.value)}
        />
        <Button variant="pink" size="sm" onClick={() => setShowAdd(v => !v)}>
          {showAdd ? "✕ Cancel" : "+ Add"}
        </Button>
      </div>
      ...
      {showAdd && (
        <div className="rounded-[14px] border p-4 mb-4" style={{ background: "var(--card2)", borderColor: "var(--border2)" }}>
          ...
          <div className="grid grid-cols-2 gap-2.5 mb-3.5">
            {[["number","Number (e.g. 107)"],["division","Division"],["studio","Studio Name"],["title","Routine Title"]].map(([key, ph]) => (
              <input key={key} placeholder={ph}
                className="h-[52px] rounded-[8px] border px-3.5 text-[14px] outline-none bg-[var(--surface)] placeholder-gray-600 focus:border-[var(--border2)]"
                style={{ borderColor: "var(--border)" }}
                value={(newR as any)[key]}
                onChange={e => setNewR(v => ({ ...v, [key]: e.target.value }))}
              />
            ))}
          </div>
        ...
        {FILTERS.map(({ key, label }) => (
          <button key={key}
            className={`h-9 px-3.5 rounded-full border text-[13px] font-medium transition-all whitespace-nowrap
              ${filter === key ? "border-pink-400/30 bg-pink-400/10 text-pink-400" : "border-[var(--border)] text-gray-500 hover:text-gray-300"}`}
      ...
      {filtered.map(r => {
        const s = getStatus(r);
        return (
          <div key={r.id} className={`rounded-[14px] border mb-2.5 overflow-hidden ${s === "completed" ? "opacity-40" : ""}`}
            style={{ background: s === "stage" ? "rgba(255,208,96,0.10)" : "var(--card)", borderColor: "var(--border)" }}>
            <div className="flex items-center gap-3.5 px-4 pt-4 pb-3">
              <div className={`font-display text-[40px] leading-none min-w-[56px] ${
                { stage:"text-yellow-300", ready:"text-emerald-400", checked:"text-amber-400", "not-here":"text-gray-600", completed:"text-gray-600" }[s]
              }`}>{r.number}</div>
              ...
              {r.has_prop && (
                <span className="font-mono text-[9px] px-2.5 py-1 rounded-full border text-orange-400"
                  style={{ background: "rgba(255,140,0,0.12)", borderColor: "rgba(255,140,0,0.35)" }}>🎬 Prop</span>
              )}
              <StatusBadge status={s} />
            </div>
            <div className="flex flex-wrap gap-2 px-4 pb-4">
              {!r.checked_in && !r.completed && <Button variant="amber" size="sm" onClick={() => checkIn(r.id)}>Check In</Button>}
              {r.checked_in  && !r.completed && <Button variant="ghost" size="sm" onClick={() => undoCheckIn(r.id)}>Undo Check-In</Button>}
            ...
            {r.check_in_time && (
              <div className="font-mono text-[10px] text-gray-600 px-4 pb-3">Checked in {r.check_in_time}</div>
            )}
          </div>
        );
      })}
