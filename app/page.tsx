...
export default function Home() {
  ...
  function handleEnter(name: string, r: "emcee" | "backstage") {
    signIn(name, r);
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        <main className="flex-1 overflow-y-auto p-4 md:p-7 transition-all duration-300"
          ...
          <div className="max-w-2xl mx-auto w-full">
            ...
            {view === "import" && (
              <ImportView
                onImport={async (rows) => { await routines.clearAll(); await routines.bulkInsert(rows as any); setView("backstage"); }}
                onReset={() => setShowReset(true)}
              />
            )}
          </div>
      ...
      {showEvents && (
        <EventsModal events={events} activeEvent={activeEvent}
          onSwitch={(e: any) => { switchEvent(e); setShowEvents(false); }}
          onCreate={async (n: string, d: string, l: string) => { await routines.clearAll(); await createEvent(n, d, l); }}
          onClose={() => setShowEvents(false)}
          onReset={() => { setShowEvents(false); setShowReset(true); }} />
      )}
      {showReset && (
        <ResetModal onConfirm={handleReset} onClose={() => setShowReset(false)} eventName={activeEvent?.name} />
      )}
    </div>
  );
}
function EventsModal({ events, activeEvent, onSwitch, onCreate, onClose, onReset }: any) {
      ...
      <div className="w-full max-w-md rounded-[18px] border p-6" style={{ background:"var(--card)", borderColor:"var(--border)" }} onClick={(e:any)=>e.stopPropagation()}>
        ...
        {!creating ? (
          <div className="flex flex-col gap-2">
            ...
            <button className="w-full h-11 rounded-[8px] border text-[13px] font-medium transition-all" style={{ borderColor:"rgba(255,82,88,0.25)", color:"#ff5258", background:"rgba(255,82,88,0.07)" }} onClick={onReset}>🗑 Reset Current Event</button>
          </div>
        ) : (
          <form onSubmit={handleCreate} className="flex flex-col gap-2.5 border-t pt-4" style={{ borderColor:"var(--border)" }}>
            ...
            {[{k:"name",v:name,s:setName,p:"Event name"},{k:"date",v:date,s:setDate,p:"Date"},{k:"location",v:location,s:setLocation,p:"Venue / city"}].map(({k,v,s,p})=>(
